    ...
    try:
        ...
        if response.status_code == 200:
            if action in {"status", "scan"}:
                print(f"\n✅ Success! {action.capitalize()} report sent to your phone.")
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("action", choices=["buy", "sell", "status", "scan"], help="Action to perform (buy, sell, status, or scan)")
